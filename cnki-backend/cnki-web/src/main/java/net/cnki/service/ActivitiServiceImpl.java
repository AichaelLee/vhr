...
import net.cnki.bean.activiti.vo.HistoricTaskInstanceVo;
import net.cnki.bean.activiti.vo.ProcessDefinitionVo;
...
//                .name(fileName)classpath:dataFile/ZH.txt
//                .deploy();
        Deployment deployment = repositoryService.createDeployment()
                .addClasspathResource("processes/mymodel1.bpmn20.xml")
                .deploy();
        //验证部署是否成功
        ProcessDefinitionQuery processDefinitionQuery = repositoryService.createProcessDefinitionQuery();
        long count = processDefinitionQuery.processDefinitionKey("mymodel1").count();
        System.out.println("qwuery list is:======"+processDefinitionQuery.list());
        List<ProcessDefinition> pds = processDefinitionQuery.list();
        if (pds != null && pds.size() > 0) {
            for (ProcessDefinition pd : pds) {
                log.info("ID:" + pd.getId() + ",NAME:" + pd.getName() + ",KEY:" + pd.getKey() + ",VERSION:" + pd.getVersion() + ",RESOURCE_NAME:" + pd.getResourceName() + ",DGRM_RESOURCE_NAME:" + pd.getDiagramResourceName());
            }
        }
    ...
    public ProcessInstance startProcessInstance(String instanceKey, Map variables) {
        ...
        log.info(String.format("id:%s,activitiId:%s", processInstance.getId(), processInstance.getActivityId()));
    ...
    public List processDefinitionQuery(String instanceKey) {
        ...
        log.info("queryProcdef query list:" + pds);
        if (pds != null && pds.size() > 0) {
            for (ProcessDefinition pd : pds) {
                log.info("ID:" + pd.getId() + ",NAME:" + pd.getName() + ",KEY:" + pd.getKey() + ",VERSION:" + pd.getVersion() + ",RESOURCE_NAME:" + pd.getResourceName() + ",DGRM_RESOURCE_NAME:" + pd.getDiagramResourceName());
            }
